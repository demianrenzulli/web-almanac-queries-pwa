...
SELECT
  client,
  COUNT(0) AS freq,
  COUNTIF(LOWER(install_events) LIKE '%appinstalled%') AS appinstalled,
  COUNTIF(LOWER(install_events) LIKE '%beforeinstallprompt%') AS beforeinstallprompt
FROM
(
  SELECT
